...
def main():
    ...
    # Center the reservoir type selection on the main page
    ...
    st.sidebar.markdown("**Note:** Please input all required parameters. Default values are set to 0.0 and must be updated.")
    
    res_p = st.sidebar.number_input("Reservoir Pressure (psi)", min_value=0.0, value=0.0)
    t = st.sidebar.number_input("Temperature (F)", min_value=0.0, value=0.0)
    sg_g = st.sidebar.number_input("Gas Specific Gravity", min_value=0.0, value=0.0)
    c_f = st.sidebar.number_input("Rock Compressibility (1/psi)", min_value=0.0, value=0.0, format="%.2e")
    c_w = st.sidebar.number_input("Water Compressibility (1/psi)", min_value=0.0, value=0.0, format="%.2e")
    S_wc = st.sidebar.number_input("Initial Water Saturation (fraction)", min_value=0.0, max_value=1.0, value=0.0)

    if reservoir_type == "Oil":
        api = st.sidebar.number_input("API Gravity", min_value=0.0, value=0.0)
        r_s_input = st.sidebar.number_input("Solution Gas-Oil Ratio (SCF/STB)", min_value=0.0, value=0.0)
        m = st.sidebar.number_input("Gas Cap Ratio (m)", min_value=0.0, value=0.0)

    # File uploader with key tied to reservoir_type to reset data on change
    ...
    if uploaded_file:
        ...
        if missing_columns:
            ...
            if reservoir_type == "Oil":  # Fixed typo here: 'reservoir Female_type' to 'reservoir_type'
            ...
            if st.checkbox("Calculate Material Balance"):
                if reservoir_type == "Oil":
                    ...
                    if not influx_present:
                        ...
                        ax.set_xlabel('E_t (RB/STB)'); ax.set_ylabel('F')
                        ...
                        ax.plot(E_t_data, N_intercept * E_t_data, 'r-', label=f'N = {N_intercept * 1e6:.2f} STB')
                        ax.set_xlabel('E_t (RB/STB)'); ax.set_ylabel('F - W_e')
                    ...
                    if not influx_present:
                        ...
                        ax.set_xlabel('Cum Gas (BSCF)'); ax.set_ylabel('F / E_t (RB)')
                        ax.set_title('Material Balance (No Influx)'); ax.legend(); ax.grid()
                        st.pyplot(fig)
                        ...
                        fig, ax = plt.subplots()
                        ax.plot(Gp_data, ratio, 'bo-', label=f'OGIP = {G_intercept:.2f} BSCF')
                        ax.set_xlabel('Cum Gas (BSCF)'); ax.set_ylabel('(F - W_e) / E_t (RB)')
                        ax.set_title('Material Balance (With Influx)'); ax.legend(); ax.grid()
                        st.pyplot(fig)
            ...
            if st.checkbox("Plot Drive Mechanism Analysis"):
                if reservoir_type == "Oil":
                    ...
                    We_term_drive = We_data / (G_intercept * 1e3) if influx_present and G_intercept != 0 else np.zeros(n_points)
                        ...
                        if future_date <= dates.iloc[-1]:
                            ...
                            if reservoir_type == "Oil":
                                ...
                                def mb_oil(p, Np, Gp, Wp):
                                    ...
                                    Bw = calculate_Bw(p, t)
                                    F = Np * 1e6 * Bo + (Gp * 1e6 - Np * 1e6 * R_si) * Bg + Wp * 1e6 * Bw
                                    E_o = (Bo - B_oi) + (R_si - calculate_Rs(p, p_b, r_s_input, sg_g, api, t)) * Bg if p < p_b else Bo - B_oi
                                    E_g = B_oi * (Bg / B_gi - 1)
